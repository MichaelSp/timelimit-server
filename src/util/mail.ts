...
const mailServerBlacklist = (process.env.MAIL_SERVER_BLACKLIST || "")
  .split(",")
  .filter((item) => !!item)

const mailSender = process.env.MAIL_SENDER || ""
const mailTransportConfig =
  JSON.parse(process.env.MAIL_TRANSPORT || "null") || undefined
const isDevMode = process.env.NODE_ENV === "development"

const mailTransport =
  isDevMode || mailTransportConfig !== undefined
    ? createTransport(
        isDevMode
          ? {
              jsonTransport: true,
...
function createMailTemplateSender(templateName: string) {
  const compileTemplate = (filename: string) =>
    compile(
      readFileSync(
        resolve(__dirname, "../../other/mail", templateName, filename),
      ).toString("utf8"),
    )
  ...
  const sendMail = async ({
    ...
    if (!mailTransport) {
      throw new Error(
        "can not send mails without mail config and without NODE_ENV=development",
      )
    }
...
export const sendAuthenticationMail = async ({
  ...
  await loginMailSender.sendMail({
    ...
    params: {
      subject:
        locale === "de" ? "Anmeldung bei TimeLimit" : "Sign in at TimeLimit",
      introtext:
        locale === "de"
          ? "Geben Sie zum Authentifizieren folgenden Code in TimeLimit ein"
          : "To authenticate, enter the following code in TimeLimit",
      code,
      outrotext:
        locale === "de"
          ? "Geben Sie diesen Code nicht an Dritte weiter."
          : "Do not share this code with third parties.",
      mailimprint,
      deviceName,
      deviceNameIntro:
        locale === "de"
          ? "Die Anmeldung wurde am Gerät"
          : "The login was attempted at the device",
...
export const sendManipulationWarningMail = async ({
  receiver,
  deviceName,
}: {
  receiver: string
...
export const sendUninstallWarningMail = async ({
  receiver,
  deviceName,
}: {
  receiver: string
...
export const sendTaskDoneMail = async ({
  receiver,
  child,
  task,
}: {
...
export const sendDeviceLinkedMail = async ({
  receiver,
  deviceName,
  locale,
}: {
  ...
  await deviceLinkedSender.sendMail({
    ...
    params: {
      ...
      postText:
        locale === "de"
          ? "über Ihre E-Mail-Adresse hinzugefügt."
          : "was added using your mail address.",
...
const passwordRecoveryUsedMailSender = createMailTemplateSender(
  "password-recovery-used",
)

export const sendPasswordRecoveryUsedMail = async ({
  receiver,
  locale,
}: {
  receiver: string
...
export function sanitizeMailAddress(input: string): string | null {
  ...
  if (typeof address !== "string") {
    throw new IllegalStateException({
      staticMessage: "mail address is not a string",
    })
  }
