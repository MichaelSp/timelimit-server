...
import {
  getCategoryAssignedApps,
  getCategoryBaseDatas,
  getCategoryDataToSync,
  getRules,
  getTasks,
  getUsedTimes,
} from "./category"
...
export const generateServerDataStatus = async ({
  ...
  const result: ServerDataStatus = {
    fullVersion: config.alwaysPro
      ? 1
      : familyEntry.hasFullVersion
        ? parseInt(familyEntry.fullVersionUntil, 10)
        : 0,
  ...
  if (familyEntry.deviceListVersion !== clientStatus.devices) {
    result.devices = await getDeviceList({
      database,
      transaction,
      familyEntry,
    })
  ...
  const categoryDataToSync = await getCategoryDataToSync({
    database,
    transaction,
    familyEntry,
    categoriesStatus: clientStatus.categories,
  })
  ...
  if (categoryDataToSync.categoryIdsToSyncAssignedApps.length > 0) {
    result.categoryApp = await getCategoryAssignedApps({
      ...
      categoryIdsToSyncAssignedApps:
        categoryDataToSync.categoryIdsToSyncAssignedApps,
    })
  }
  ...
  if (
    categoryDataToSync.categoryIdsToSyncTasks.length > 0 &&
    doesClientSupportTasks
  ) {
