...
export const setPrimaryDevice = async ({ database, websocket, deviceAuthToken, currentUserId, action }: {
  ...
  return database.transaction(async (transaction): Promise<'assigned to other device' | 'requires full version' | 'success'> => {
    ...
    if ((deviceEntry.currentUserId !== currentUserId) || (currentUserId === '')) throw new Conflict()

    const userEntryUnsafe = await transaction.legacy.database.user.findOne({
      where: { familyId: deviceEntry.familyId, userId: deviceEntry.currentUserId },
      transaction: transaction.legacy.transaction,
      attributes: ['currentDevice']
    })
    if (!userEntryUnsafe) throw new Conflict()
    const userEntry = { currentDevice: userEntryUnsafe.currentDevice }

    const userDeviceEntriesUnsafe = await transaction.legacy.database.device.findAll({
      where: { familyId: deviceEntry.familyId, currentUserId },
      transaction: transaction.legacy.transaction,
      attributes: ['deviceId']
    })
    const userDeviceEntries = userDeviceEntriesUnsafe.map((item) => ({ deviceId: item.deviceId }))

    if (userDeviceEntries.length >= 2) {
      const familyEntryUnsafe = await transaction.legacy.database.family.findOne({
        where: { familyId: deviceEntry.familyId },
        transaction: transaction.legacy.transaction,
        attributes: ['hasFullVersion']
      })
      if (!familyEntryUnsafe) throw new Conflict()
      if (!(familyEntryUnsafe.hasFullVersion || config.alwaysPro)) return 'requires full version'
    }

    if (action === 'set this device') {
      if (userDeviceEntries.find((item) => item.deviceId === userEntry.currentDevice)) return 'assigned to other device'
      const [affectedRows] = await transaction.legacy.database.user.update({ currentDevice: deviceEntry.deviceId }, {
        where: { familyId: deviceEntry.familyId, userId: currentUserId },
        transaction: transaction.legacy.transaction
      })
      if (affectedRows !== 1) throw new Conflict()
    } else if (action === 'unset this device') {
      if (userEntry.currentDevice !== deviceEntry.deviceId) throw new Conflict()
      const [affectedRows] = await transaction.legacy.database.user.update({ currentDevice: '' }, {
        where: { familyId: deviceEntry.familyId, userId: currentUserId },
        transaction: transaction.legacy.transaction
      })
      if (affectedRows !== 1) throw new Conflict()
    ...
    await transaction.legacy.database.family.update({ userListVersion: generateVersionId() }, {
      transaction: transaction.legacy.transaction,
      where: { familyId: deviceEntry.familyId }
    })
